...
Also serves GPU stats, server logs, and writes JSONL for analytics/replay
"""
import http.server
import http.client
import json
...
def get_jsonl_file():
    ...
    if jsonl_file is None:
        os.makedirs(os.path.dirname(JSONL_LOG_FILE), exist_ok=True)
        jsonl_file = open(JSONL_LOG_FILE, 'a', buffering=1)
    return jsonl_file

def write_jsonl(req_id, method, path, headers, body, response_status, response_headers, response_body, duration):
    record = {
...
def store_request(req_id, method, path, headers, body, response_status, response_headers, response_body, duration):
    write_jsonl(req_id, method, path, headers, body, response_status, response_headers, response_body, duration)
...
class ViewerHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        elif self.path == '/api/server-log':
            self.send_response(200)
            self.send_header('Content-Type', 'text/plain')
            self.send_header('Access-Control-Allow-Origin', '*')
            self.end_headers()
            try:
                result = subprocess.run(['tail', '-n', '100', SERVER_LOG_FILE], capture_output=True, text=True)
                self.wfile.write(result.stdout.encode())
            except Exception as e:
                self.wfile.write(f"Error reading log: {e}".encode())
        elif self.path == '/api/jsonl':
            self.send_response(200)
            self.send_header('Content-Type', 'application/jsonl')
            self.send_header('Access-Control-Allow-Origin', '*')
            self.send_header('Content-Disposition', f'attachment; filename="requests.jsonl"')
            self.end_headers()
            try:
                ...
                self.wfile.write(f"Error reading jsonl: {e}".encode())
        else:
...
class GPUStatsHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/' or self.path == '/index.html':
            self.send_response(200)
            self.send_header('Content-Type', 'text/html')
            self.end_headers()
            self.wfile.write(GPU_HTML.encode())
            ...
            try:
                result = subprocess.run(
                    ['nvidia-smi', '--query-gpu=index,name,temperature.gpu,utilization.gpu,utilization.memory,memory.used,memory.total,power.draw,clocks.sm',
                ...
                self.wfile.write(json.dumps(gpus).encode())
            except Exception as e:
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', monospace; background: #0d1117; color: #0f0; padding: 20px; }
        h1 { color: #0f0; margin-bottom: 20px; font-size: 20px; }
        table { border-collapse: collapse; width: 100%; background: #161b22; border-radius: 8px; overflow: hidden; }
        th, td { border: 1px solid #30363d; padding: 12px; text-align: left; }
        th { background: #21262d; color: #58a6ff; font-weight: 600; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; }
        td { font-size: 14px; }
        .temp-hot { color: #f85149; }
        .temp-warm { color: #d29922; }
        .temp-cool { color: #3fb950; }
        .util-high { color: #3fb950; }
        .util-low { color: #8b949e; }
    </style>
</head>
<body>
    <h1>🔥 GPU Stats</h1>
    <table>
        <thead><tr><th>GPU</th><th>Name</th><th>Temp</th><th>GPU %</th><th>Mem %</th><th>VRAM</th><th>Total</th><th>Power</th><th>Clock</th></tr></thead>
        <tbody id="gpu-data"></tbody>
    </table>
    <script>
        fetch('/api/gpu').then(r => r.json()).then(data => {
            const tbody = document.getElementById('gpu-data');
            if (data.error) { tbody.innerHTML = '<tr><td colspan="9" style="color:#f85149">Error: ' + data.error + '</td></tr>'; return; }
            tbody.innerHTML = data.map(g => {
                const temp = parseInt(g.temp);
                const tempClass = temp > 80 ? 'temp-hot' : (temp > 60 ? 'temp-warm' : 'temp-cool');
                const util = parseInt(g.gpu_util);
                const utilClass = util > 50 ? 'util-high' : 'util-low';
                return '<tr><td>' + g.index + '</td><td>' + g.name + '</td><td class="' + tempClass + '">' + g.temp + '°C</td><td class="' + utilClass + '">' + g.gpu_util + '%</td><td>' + g.mem_util + '%</td><td>' + g.mem_used + ' MB</td><td>' + g.mem_total + ' MB</td><td>' + g.power + ' W</td><td>' + g.clock + ' MHz</td></tr>';
            }).join('');
        });
    </script>
</body>
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .clear-btn { background: #da3633; border-color: #da3633; color: white; }
        .clear-btn:hover { background: #f85149; }
        .download-btn { background: #1f6feb; border-color: #1f6feb; color: white; margin-left: auto; }
...
<body>
    <div id="header">
        ...
        <div class="subtitle">
            Proxy on port 18000 | <a href="http://localhost:9001">GPU Stats</a> | 
            <a href="/api/jsonl" download="requests.jsonl">Download JSONL</a>
        </div>
        <div id="stats">
            ...
            <div class="stat"><span>JSONL:</span><span id="jsonlSize">-</span></div>
    ...
    <div id="filters">
        ...
        <button class="filter-btn download-btn" onclick="downloadJsonl()">Download JSONL</button>
    </div>
    ...
    <script>
        ...
        document.querySelectorAll('.filter-btn:not(.clear-btn):not(.download-btn)').forEach(btn => {
        ...
        function downloadJsonl() {
            window.location.href = '/api/jsonl';
        }
        ...
        function renderRequests() {
            ...
            if (filtered.length === 0) {
                container.innerHTML = '<div class="empty-state"><h2>No requests</h2><p>Make a request to localhost:18000</p></div>';
                return;
            }
...
def run_viewer():
    ...
    print(f'JSONL log file: {JSONL_LOG_FILE}')
    server.serve_forever()

def run_gpu_stats():
    server = http.server.HTTPServer(('0.0.0.0', GPU_STATS_PORT), GPUStatsHandler)
    print(f'GPU stats server running on port {GPU_STATS_PORT}')
    server.serve_forever()
...
if __name__ == '__main__':
    ...
    print(f'  GPU:    http://localhost:{GPU_STATS_PORT}')
    print(f'  JSONL:  {JSONL_LOG_FILE}')
    
    proxy_thread = threading.Thread(target=run_proxy, daemon=True)
    proxy_thread.start()
    
    gpu_thread = threading.Thread(target=run_gpu_stats, daemon=True)
    gpu_thread.start()
